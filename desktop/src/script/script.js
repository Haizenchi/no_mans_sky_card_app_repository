/* NMS Passport Desktop v1.0.0 — Web core v2.2 — Haizenchi — bilingual FR/EN */
(() => {
'use strict';
...
const I18N = {
  en:{
    ...
    msg:{imageFormat:'Unsupported image format. Use JPG, PNG or WEBP.',imageTooLarge:'Image is too large (18 MB max).',imageInvalid:'Invalid image or excessive dimensions.',secureGenerationUnavailable:'Secure generation is unavailable in this browser.',pngExported:'PNG {w} × {h} exported.',jsonExported:'JSON profile exported.',jsonImported:'JSON profile imported and validated.',jsonInvalid:'Invalid, oversized or non-compliant JSON file.',profilePrompt:'Local profile name:',defaultProfile:'My Traveller',profileSaved:'Profile saved locally.',profileLoaded:'Profile “{name}” loaded.',selected:'selected',deleteProfileConfirm:'Delete profile “{name}”?',profileDeleted:'Profile deleted.',newSessionConfirm:'Create a blank session? Unsaved data will be lost.',newSession:'New blank session.',linkCopied:'Share link copied.',dataCopied:'Card data copied without a local path.',shareError:'Unable to create or copy sharing data.',saveProfileError:'Unable to save profile.',resetConfirm:'Reset the “{module}” module?',moduleReset:'Module reset.',sharedLoaded:'Shared card loaded from the link.',desktopSaveError:'Unable to save the file.'}
  },
  fr:{
    ...
    msg:{imageFormat:'Format image refusé. Utilise JPG, PNG ou WEBP.',imageTooLarge:'Image trop volumineuse (18 Mo max).',imageInvalid:'Image invalide ou dimensions excessives.',secureGenerationUnavailable:'Génération sécurisée indisponible dans ce navigateur.',pngExported:'PNG {w} × {h} exporté.',jsonExported:'Profil JSON exporté.',jsonImported:'Profil JSON importé et validé.',jsonInvalid:'Fichier JSON invalide, trop volumineux ou non conforme.',profilePrompt:'Nom du profil local :',defaultProfile:'Mon voyageur',profileSaved:'Profil enregistré localement.',profileLoaded:'Profil « {name} » chargé.',selected:'sélectionné',deleteProfileConfirm:'Supprimer le profil « {name} » ?',profileDeleted:'Profil supprimé.',newSessionConfirm:'Créer une session vierge ? Les données non enregistrées seront perdues.',newSession:'Nouvelle session vierge.',linkCopied:'Lien partageable copié.',dataCopied:'Données de fiche copiées sans chemin local.',shareError:'Impossible de créer ou copier les données de partage.',saveProfileError:'Impossible d’enregistrer le profil.',resetConfirm:'Réinitialiser le module « {module} » ?',moduleReset:'Module réinitialisé.',sharedLoaded:'Fiche partagée chargée depuis le lien.',desktopSaveError:'Impossible d’enregistrer le fichier.'}
  }
};
...
function updateShareUi(){const b=$('copyShare');if(!b)return;const publicContext=isPublicShareContext();b.textContent=publicContext?t('ui.copyLink'):t('ui.copyData');b.title=publicContext?t('ui.copyLink'):t('ui.copyData');}
...
function isDesktopApp(){return Boolean(window.__TAURI__&&window.__TAURI__.dialog&&window.__TAURI__.fs);}
async function saveBlob(blob,filename){
  if(isDesktopApp()){
    try{
      const ext=(filename.split('.').pop()||'').toLowerCase();
      const filters=ext==='png'?[{name:'PNG Image',extensions:['png']}]:[{name:'JSON',extensions:['json']}];
      const path=await window.__TAURI__.dialog.save({defaultPath:filename,filters});
...
function setupEvents(){
  ...
  $('download').addEventListener('click',()=>downloadPng().catch(e=>{console.error(e);setStatus(t('msg.desktopSaveError'));}));$('exportJson').addEventListener('click',()=>exportJson().catch(e=>{console.error(e);setStatus(t('msg.desktopSaveError'));}));$('importJson').addEventListener('change',e=>{const f=e.target.files?.[0];if(f)importJson(f);e.target.value='';});$('copyShare').addEventListener('click',copyShare);
  ...
  const pwaButton=$('installPwa');if(!isDesktopApp()&&pwaButton){window.addEventListener('beforeinstallprompt',e=>{e.preventDefault();installPrompt=e;pwaButton.hidden=false;});pwaButton.addEventListener('click',async()=>{if(!installPrompt)return;installPrompt.prompt();await installPrompt.userChoice;installPrompt=null;pwaButton.hidden=true;});}else if(pwaButton){pwaButton.hidden=true;}
}

function registerPwa(){if(isDesktopApp())return;const localHost=['localhost','127.0.0.1','[::1]'].includes(location.hostname);if('serviceWorker' in navigator&&(location.protocol==='https:'||(location.protocol==='http:'&&localHost)))navigator.serviceWorker.register('sw.js',{scope:'./'}).catch(e=>console.warn('SW',e));}
async function init(){applyLanguage();bindForms();setupEvents();updateShareUi();const shared=applySharedHash();syncFormFromState();setActiveModule(activeModule);await hydrateImages();await refreshProfiles();registerPwa();if(shared)setStatus(t('msg.sharedLoaded'),4000);console.info(`NMS Passport v${APP_VERSION} — ${APP_AUTHOR}`);}
